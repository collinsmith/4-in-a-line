...
public class Gomoku implements Runnable {
	private static final Heuristic DEFAULT = new Heuristic() {
		@Override
		public int evaluate(Node state, byte piece) {
			return 0;
		}
	};
	...
	public Gomoku(Heuristic heuristic) {
	...
	private static interface Heuristic {
		int evaluate(Node state, byte piece);
	}
	...
	private static class IDSABSearch implements Callable<Integer> {
		...
		public IDSABSearch(Gomoku game, Node initialState, Heuristic heuristic) {
			this.GAME = game;
		...
		private int min(Node currentState, int i, int j, byte piece, int alpha, int beta, int depth) {
			...
			if (winner != UNKNOWN || depthLimit <= depth) {
				return HEURISTIC.evaluate(successor, piece);
				/*if (winner == piece) {
					return 10000;
				} else if (winner == DRAW) {
					return 0;
	...
	private static class Node {
		...
		Node(long oLocs, long xLocs) {
			...
			int hash = 1;
			hash = 31 * hash + Long.hashCode(O_LOCS);
			hash = 31 * hash + Long.hashCode(X_LOCS);
			this.HASH = hash;
		}
